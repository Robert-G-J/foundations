...
object EitherExercises1 {
  ...
  case class UserNotFound(id: UserId) extends UserEmailError {
    def errorMessage: String = s"User ${id.value} is missing"
  }
  case class EmailNotFound(id: UserId) extends UserEmailError {
    def errorMessage: String = s"User ${id.value} has no email address"
  }
